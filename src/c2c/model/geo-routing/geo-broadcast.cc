...
//      result.route->SetOutputDevice ();
  ...
  //Vineet
     //   double distanceReceiver = DistanceInMeters (m_posvector.Lat, m_posvector.Long, daddr->GetGeoAreaPos1 ()->lat, daddr->GetGeoAreaPos1 ()->lon);
...
//      std::cout<<"GeoBroadcast RouteInput: Node "<< m_c2c->GetObject<Node> ()->GetId ()<<" is NOT in the destination area "<<std::endl;
     ...
     //Vineet
     //   double distanceLastF = DistanceInMeters (header.GetSourPosVector().Lat, header.GetSourPosVector().Long, daddr->GetGeoAreaPos1 ()->lat, daddr->GetGeoAreaPos1 ()->lon);
...
//        std::cout<<"GeoBroadcast RouteInput: Discard message "<<std::endl;
     ...
     else
     {
       ...
       //Vineet
       //double distanceM = DistanceInMeters (vector.Lat, vector.Long, daddr->GetGeoAreaPos1 ()->lat, daddr->GetGeoAreaPos1 ()->lon);
