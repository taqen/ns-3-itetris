...
{
#ifdef WIN32
	bool ServerSocket::init_windows_sockets_ = true;
	bool ServerServerSocket::windows_sockets_initialized_ = false;
	int ServerServerSocket::instance_count_ = 0;
#endif
		...
		const throw()
	{
		...
		if (r < 0)
			BailOnSocketError("tcpip::ServerSocket::datawaiting @ select");
		...
		throw( SocketException )
	{
		...
		if( server_socket_ < 0 )
		{
			...
			if( server_socket_ < 0 )
				BailOnSocketError("tcpip::ServerSocket::accept() @ socket");
			...
			if ( bind(server_socket_, (struct sockaddr*)&self, sizeof(self)) != 0 )
				BailOnSocketError("tcpip::ServerSocket::accept() Unable to create listening socket");


			// Make it a "listening socket"
			if ( listen(server_socket_, 10) == -1 )
				BailOnSocketError("tcpip::ServerSocket::accept() Unable to listen on server socket");
		...
		throw(SocketException )
	{
		...
		if( server_socket_ > 0 )
		{
		    ...
		    if (ioctlsocket(server_socket_, FIONBIO, &NonBlock) == SOCKET_ERROR)
				BailOnSocketError("tcpip::ServerSocket::set_blocking() Unable to initialize non blocking I/O");
#else
		...
		throw( SocketException )
	{
		...
		if( !atoaddr( host_.c_str(), addr) )
			BailOnSocketError("tcpip::ServerSocket::connect() @ Invalid network address");
		...
		if( socket_ < 0 )
			BailOnSocketError("tcpip::ServerSocket::connect() @ socket");

		if( ::connect( socket_, (sockaddr const*)&address, sizeof(address) ) < 0 )
			BailOnSocketError("tcpip::ServerSocket::connect() @ connect");
		...
		throw( SocketException )
	{
		...
		if( a <= 0 )
			BailOnSocketError
			( "tcpip::ServerSocket::receive() @ recv" );
		...
		throw( SocketException )
	{
		...
		while (bytesRead<4)
		{
			...
			if( readThisTime <= 0 )
				BailOnSocketError( "tcpip::ServerSocket::receive() 1 @ recv" );
		...
		while (bytesRead<NN)
		{
			...
			if( readThisTime <= 0 )
				BailOnSocketError( "tcpip::ServerSocket::receive() 2 @ recv" );
