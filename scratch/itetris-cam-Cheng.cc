...
main (int argc, char *argv[])
{
   ...
   * In this example, the configTechnologiesCheng.xml is used. This configuration file calls the confWaveVehicleCheng.xml configuration
   * file to install on vehicles ITS G5A Netdevices that communicate over a propagation channel modeled with the Cheng&Stancil model for 
   * Highway scenarios. 
   */
...
//   LogComponentEnable ("WaveInstaller",LOG_LEVEL_DEBUG);
  ...
  if (fileConfTechnologies == "")
    {
      fileConfTechnologies = "scratch/configTechnologiesCheng.xml"; // change this
    }
 ...
 //  Simulator::Schedule (Seconds (8.0), &ReceivePackets,packetManager,0);
  Simulator::Schedule (Seconds (80.0), &ReceivePackets,packetManager,1);
